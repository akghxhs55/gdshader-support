...
object GdsLookupElements {
    ...
    val UNIFORM_GROUP_KEYWORD =
        LookupElementBuilder
            .create("group_uniforms")
            .withBoldness(true)
            .withPriority(PRIORITY_NORMAL)
    ...
    val CONTROL_STATEMENT_STARTERS =
        listOf("if", "for", "while", "switch").map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
    ...
    val RENDER_MODES =
        GdsKeywords.RENDER_MODES.values.flatten().map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
                .withIcon(AllIcons.Nodes.Enum)
    ...
    val STENCIL_MODES =
        GdsKeywords.STENCIL_MODES.values.flatten().map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
                .withIcon(AllIcons.Nodes.Enum)
    ...
    val DECLARABLE_BUILTIN_TYPES =
        Builtins.ALL_DATA_TYPE_LIST.filter { it.isDeclarable }.map {
            LookupElementBuilder
                .create(it.presentationText)
                .withBoldness(true)
    ...
    val OPAQUE_BUILTIN_TYPES =
        Builtins.ALL_DATA_TYPE_LIST.filter { it.isOpaque }.map {
            LookupElementBuilder
                .create(it.presentationText)
                .withBoldness(true)
                .withIcon(AllIcons.Nodes.Type)
    ...
    val RETURNABLE_BUILTIN_TYPES =
        Builtins.ALL_DATA_TYPE_LIST.filter { it.isValidReturnType }.map {
            LookupElementBuilder
                .create(it.presentationText)
                .withBoldness(true)
                .withIcon(AllIcons.Nodes.Type)
    ...
    val BUILTIN_FUNCTIONS =
        Builtins.FUNCTIONS.mapValues {
            it.value.map { functionSpec ->
                LookupElementBuilder
                    .create(functionSpec.name)
                    .withBoldness(true)
                    ...
                    .withInsertHandler(
                        if (functionSpec.parameters.isEmpty()) ParenthesesInsertHandler.NO_PARAMETERS else ParenthesesInsertHandler.WITH_PARAMETERS,
                    ).withPriority(PRIORITY_BUILTIN)
            }
        }

    val BUILTIN_VARIABLES =
        Builtins.VARIABLES.mapValues {
    ...
    val PROCESSING_FUNCTIONS_WITHOUT_RETURN_TYPE =
        Builtins.PROCESSING_FUNCTIONS.mapValues {
            it.value.map { functionContext ->
                LookupElementBuilder
                    .create(functionContext.text)
    ...
    val CONSTRUCTORS =
        Builtins.ALL_DATA_TYPE_LIST
            .filter { it.isInstantiable }
            .flatMap { type ->
                (type as? Instantiable)?.getConstructors().orEmpty().map { constructorSpec ->
                    LookupElementBuilder
                        .create(constructorSpec, constructorSpec.name)
                        .withBoldness(true)
                        .withIcon(AllIcons.Nodes.Function)
                        .appendTailText(
                            "(" +
                                constructorSpec.parameters.joinToString(
                                    ", ",
                                ) { param -> "${param.presentationTypeText} ${param.name}" } + ")",
                            true,
                        ).withInsertHandler(
                            if (constructorSpec.parameters.isEmpty()) ParenthesesInsertHandler.NO_PARAMETERS else ParenthesesInsertHandler.WITH_PARAMETERS,
                        ).withPriority(PRIORITY_LOW)
                }
            }

    val INTEGER_TYPE_CONSTRUCTORS =
        Builtins.ALL_DATA_TYPE_LIST
    ...
    val BOOLEAN_LITERALS =
        GdsKeywords.BOOLEAN_LITERALS.map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
                .withInsertHandler(AddSpaceInsertHandler(true))
    ...
    val INTERPOLATIONS =
        GdsKeywords.INTERPOLATIONS.map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
                .withInsertHandler(AddSpaceInsertHandler(true))
    ...
    val PARAMETER_QUALIFIERS =
        GdsKeywords.PARAMETER_QUALIFIERS.map {
            LookupElementBuilder
                .create(it)
                .withBoldness(true)
                .withInsertHandler(AddSpaceInsertHandler(true))
    ...
    fun createFromVariableNameDecl(nameDecl: GdsVariableNameDecl): LookupElement? {
        ...
        val builder =
            LookupElementBuilder
                .create(nameDecl, variableSpec.name)
                .withBoldness(true)
                .withTypeText(variableSpec.presentationTypeText, true)
                .withIcon(icon)
    ...
    fun createFromFunctionNameDecl(nameDecl: GdsFunctionNameDecl): LookupElement? {
        ...
        val builder =
            LookupElementBuilder
                .create(nameDecl, functionSpec.name)
                .withBoldness(true)
                .withIcon(AllIcons.Nodes.Function)
                .appendTailText(
                    "(" + functionSpec.parameters.joinToString(", ") { param -> "${param.presentationTypeText} ${param.name}" } + ")",
