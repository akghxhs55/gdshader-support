        ...
        for (virtualFile in virtualFiles) {
            ...
            for (item in items) {
                ...
                topLevel.uniformDeclaration
                    ...
                    ?.name
                    ?.let { names.add(it) }
                topLevel.constantDeclaration?.constantDeclaratorList?.constantDeclaratorList?.forEach {
                    it.variableNameDecl.name.let { name ->
                        names.add(name)
                    }
                }
    ...
    override fun getItemsByName(
        ...
        for (virtualFile in virtualFiles) {
            ...
            for (item in items) {
                ...
                topLevel.constantDeclaration?.constantDeclaratorList?.constantDeclaratorList?.forEach {
                    if (it.variableNameDecl.name ==
                        name
                    ) {
                        result.add(it.variableNameDecl as NavigationItem)
                    }
                }
